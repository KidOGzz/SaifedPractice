...
public class ChatUtils {
    ...
    public ChatUtils(String licenseKey, String validationServer, Plugin plugin) {
        this.licenseKey = licenseKey;
        this.plugin = plugin;
        this.validationServer = validationServer;
    }

    public ChatUtils setSecurityKey(String securityKey) {
        this.securityKey = securityKey;
        return this;
    }

    public ChatUtils setConsoleLog(LogType logType) {
        this.logType = logType;
        return this;
    }

    public ChatUtils debug() {
        debug = true;
        return this;
    }

    public boolean register() {
        CC.log("&b[]==========[&eLicense-System&b]==========[]");
        CC.log("&bVerifying your license");
        ValidationType vt = isValid();
        if (vt == ValidationType.VALID) {
            CC.log("&d" + PotPvPSI.getInstance().getConfig().getString("license") + " &aLicense code is valid!");
            CC.log("&b[]==========[&eLicense-System&b]==========[]");
            return true;
        } else {
            CC.log( "&d" + PotPvPSI.getInstance().getConfig().getString("license") + " &aLicense code is &cNOT valid!");
            CC.log( "&cFailed as a result of " + vt.toString());
            CC.log( "&cDisabling plugin!");
            CC.log("&b[]==========[&eLicense-System&b]==========[]");
